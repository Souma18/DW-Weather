...
from transform.models import (
    ...
    FactHeavyRain, FactThunderstorm, FactFog, FactGale, FactCycloneTrack, FactCycloneForecast
)
...
def get_or_create_cyclone(session, clean_obj, logger, source_name=None):
    cyclone = None
    if hasattr(clean_obj, 'storm_id') and clean_obj.storm_id:
        cyclone = session.query(DimCyclone).filter(DimCyclone.storm_id == clean_obj.storm_id).first()
    
    if cyclone is None and hasattr(clean_obj, 'name') and clean_obj.name:
        cyclone = session.query(DimCyclone).filter(DimCyclone.name == clean_obj.name).first()
        
    if cyclone is None:
        name = getattr(clean_obj, 'name', None) or getattr(clean_obj, 'tc_name', None)
        cyclone = DimCyclone(
            name=name,
...
def transform_tc(clean_obj, session, logger, source_name=None):
    ...
    cyclone.intensity_category = clean_obj.intensity_category
    cyclone.start_time = clean_obj.start
    cyclone.latest_time = clean_obj.latest
    cyclone.same = clean_obj.same
    cyclone.center_id = clean_obj.centerid
    cyclone.gts = clean_obj.gts
    cyclone.updatedAt = datetime.now()
...
def transform_cyclone_track(clean_obj, session, logger, source_name=None):
    ...
    track = FactCycloneTrack(
        ...
        center_id=str(clean_obj.center_id),
        createdAt=datetime.now(),
    )
    session.add(track)
